...
from django_backend.models import Meeting, ChatRoom, ChatMessage
from fastapi_app.schemas.chat_schemas import MeetingCreate
from fastapi_app.schemas.meet_schemas import MeetingRead 
...
@router.post("/{room_id}/create")
async def create_meeting_from_chat(
    room_id: int, 
    data: MeetingCreate, 
    current_user: User = Depends(get_current_user)
):
        ...
        try:
            with transaction.atomic():
                ...
                msg = ChatMessage.objects.create(
                    room=room,
                    sender=current_user,
                    content=f" Started a {meeting.get_call_type_display()}: {meeting.title}. Join Code: {meeting.meeting_code}",
                    message_type='SYSTEM'
                )
    ...
    meeting, error = await perform_db_operations()
    
    if error:
        status_code = 403 if error == "FORBIDDEN" else 400
    ...
    return {"status": "success", "meeting_code": meeting.meeting_code}